...
def test_part1a_C():
    ...
    assert np.all(np.isclose(roadster.consumption(dist_array),
                             ref_array)), 'consumption(np.array([0.1,23.7,48.53,201.3])) not close to reference array'


### PART 1B, velocity ###
def test_part1b_A():
    ref_value_anna_21p4 = 103.58230081237032
    assert np.isclose(roadster.velocity(21.4, 'speed_anna'),
                      ref_value_anna_21p4), 'velocity(21.4) not close to reference value'


def test_part1b_B():
    ref_value_elsa_14p53 = 89.22383224404504
    assert np.isclose(roadster.velocity(14.53, 'speed_elsa'),
                      ref_value_elsa_14p53), 'velocity(14.53) not close to reference value'


def test_part1b_C():
    dist_array = np.array([0, 2.3, 4.53])
    ref_array_anna = np.array([5.0, 51.20013042688889, 78.3714678653808])
    assert np.all(np.isclose(roadster.velocity(dist_array, 'speed_anna.npz'),
                             ref_array_anna)), 'velocity(np.array([0,2.3,4.53])) not close to reference value'
...
def test_part1b_D():
    ...
    assert np.all(np.isclose(roadster.velocity(dist_array, 'speed_elsa.npz'),
                             ref_array_elsa)), 'velocity(np.array(...)) not close to reference value'
...
def test_part4a_A():
    ...
    ref_value = t0
    assert np.isclose(ref_value,
                      time_h[0]), 'first value in time_h vector should be t0, since that is the starting time'
...
def test_part4a_B():
    ...
    ref_value = 0
    assert np.isclose(ref_value, distance_km[
        0]), 'first value in distance_km vector should be 0, since no distance have been traveled yet'
...
def test_part4a_C():
    ...
    ref_value = 60
    assert np.isclose(ref_value, distance_km[
        -1]), 'last value in distance_km vector should be 60, since that is the length of the route'
...
def test_part4a_D():
    ...
    assert np.isclose(ref_value_dist_m2,
                      distance_km[-2]), 'next to last value in distance_km vector different from reference value'
    # check first value in speed_kmph
    ref_value_speed_0 = 8.2800000000000011369
    assert np.isclose(ref_value_speed_0,
                      speed_kmph[0]), 'first value in speed_kmph vector different from reference value'
    # check last value in speed_kmph
    ref_value_speed_m1 = 34.824801684943381019
    assert np.isclose(ref_value_speed_m1,
                      speed_kmph[-1]), 'last value in speed_kmph vector different from reference value'
